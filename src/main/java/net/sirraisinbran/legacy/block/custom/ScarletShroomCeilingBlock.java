...
public class ScarletShroomCeilingBlock extends OtherworldMushroomPlant {

    protected static final VoxelShape SHAPE = Block.createCuboidShape(5.0D, 6.5D, 5.0D, 11.0D, 16.0D, 11.0D);

    public ScarletShroomCeilingBlock(Settings settings, Supplier<RegistryEntry<? extends ConfiguredFeature<?, ?>>> feature) {
    ...
    protected boolean canPlantUnder(BlockState floor, BlockView world, BlockPos pos) {
        return floor.isIn(LegacyMod.SOIL) || floor.isOf(LegacyBlocks.DRY_SAND) || floor.isOf(LegacyBlocks.LUSH_GABBRO) || floor.isOf(LegacyBlocks.DEAD_LUSH) || floor.isOf(LegacyBlocks.DEAD_LUSH_GABBRO) || floor.isOf(LegacyBlocks.GABBRO);
    }
